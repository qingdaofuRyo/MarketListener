...
def test_scanner_groups_periods_reads_latest_close_and_skips_known_names(tmp_path: Path) -> None:
    ...
    index = next(item for item in items if item["sourceCode"] == "62#000300")
    assert index["periods"] == ["1d", "5m"]
    assert index["latestClose"] == 4501.5
    assert index["lastBarAt"] == "2026-08-28T10:30:00+08:00"
    assert index["classificationStatus"] == "PENDING_REVIEW"
