...
test("R4续3 supersedes nested bars with independently scaled volume bars and secondary line", () => {
  const [volume, secondary] = subchartSeries([{volume: 10, amount: 1e12}, {volume: null, amount: null}], "amount", ["red"], "blue");
  expect(volume.type).toBe("bar");
  expect(secondary.type).toBe("line");
  expect(volume.yAxisIndex).not.toBe(secondary.yAxisIndex);
  expect(secondary.z).toBeGreaterThan(volume.z!);
  expect(secondary.data).toEqual([1e12, null]);
});
...
test("R4续2 all-market uses independent route, opaque list header and canvas overlay", async ({ page }) => {
  ...
  await expect(topPeriod).toBeVisible();
  // Selection loads asynchronously and may replace the initial chart instance after reload.
  await expect.poll(async () => (await topPeriod.boundingBox())?.width || 0).toBeGreaterThan(45);
  await expect.poll(async () => (await topPeriod.boundingBox())?.width || Infinity).toBeLessThan(90);
