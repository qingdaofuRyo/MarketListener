...
## R4 组合策略与桌面布局（2026-09-17）

- 网页客户端导航包含F10，后端导航不再重复展示；Android不变。全部行情排序从倒序开始，第二次顺序、第三次恢复数据层原序。两页14字段共用QuoteValues固定槽位，压缩标签槽和数值间距，不改变行情口径。
- 组合策略编辑器要求attention、position、timing三函数齐全；Python形状代码通过受限AST解释器执行，仅允许纯表达式、局部赋值及预定义精确版本函数调用，无exec/eval、导入、属性访问、循环或任意历史索引。函数依赖保存版本及定义哈希。
- `/api/composites`提供定义校验、版本编辑、可恢复删除与扫描；数据保存在本地`strategies/composites`，旧signals定义与API作为历史兼容保留，不把四类独立信号自动启用为组合策略。
- 状态按组合版本与标的隔离：关注后才计算仓位/择时，开仓后才能加减平；平仓后仍关注可再次开仓，取消关注停止观察而不是伪造真实平仓。首次仅检查最新已结束bar，后续按游标消费；超过500根缺口不静默跳过。单标的批次失败不提交半成品状态。
- 缺失或无效结束时间的分钟bar用周期推算；日及以上无可靠结束时间时在周期边界后额外延迟一天确认，避免提前消费海外行情。此为保守降级，可能延迟观察，并非实时监控。显式is_partial不消费。
- 仓位输出保留缺失态；无真实保证金率不编造资金使用率，无观察样本不编造胜率。同期同向比较必须同组合版本、同参考起止时间、同方向，不等同因果关系。
- TargetMarket store持有跨页面监控、每批筛选快照和请求序号；旧加载/扫描结果不能覆盖新状态，编辑后停止旧扫描并刷新。目标页显示组合关注、配置指标、同期标的与择时事件；图表仅把开加减平事件作为操作标记。

## R4 管理网页更新（2026-09-12）

- 根路由显示“仪表盘”，保留受控操作入口；任务队列独立组件在日志页展示并支持刷新、取消排队任务，事件日志仍在同页。F10位置及移动端不变。
- 数据源页为只读中文库存面板，不再提供主要/备用/自定义来源设置；后端既有路由选择及兼容配置API未改变。
- 库存API增量返回storage：按清单唯一文件路径统计正式行情文件逻辑大小，共享分类只计算一次，越界/不可读取文件计入缺失。不扫描全量K线；无清单与空清单分别表示未知和零。
- 来源卡片仅消费最新行情记录来源证据；数据集登记行数不能证明来源归属。容量、K线条数、数据起止日期各有独立口径，均不承诺期间连续完整。卡片、占比图、表格复用现有主题；没有引入Grafana服务。
...
## 2026-09-11 - R4 TradingView视觉与回放续期

- `chartLayout`集中蜡烛80%/24px与成交量90%/28px宽度、显式价格/副图tick域和按侧测量的轴留白。镜像价格轴共用domain/interval；绘图边界使用同一价格域，不让缩放后轴与图形坐标分离。
- `ChartMenu`是类型与绘图组共用的Teleport菜单，支持边界定位、键盘和按关闭原因恢复焦点。类型仍为现有五种，不引入参考站额外能力。
- `useChartReplay`管理关闭/选点/暂停/播放/结束及计时器；`chartReplay`计算可见范围与至少五个逻辑空槽。真实bars、指标请求与tooltip只消费揭示前缀，空槽不属于市场数据。手动浏览暂停跟随，退出恢复原视窗，切标的/周期重置；隐藏页面暂停，卸载释放。
- 指标API暖机窗口只允许向请求起点前扩展；计算输入再次裁切至请求终点，避免底层为涨幅前值多取一条时泄漏回放未来数据。公开接口和数据契约不变。
