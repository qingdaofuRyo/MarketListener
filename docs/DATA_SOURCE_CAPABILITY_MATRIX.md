...
| 国内期货品种市值结构 | PARTIAL | `product-notional` 只使用商品期货有效月份合约的 `结算价 × 同交易日交易乘数 × 单边持仓量`；2026-09-01 本机构建覆盖 2023-09-05～2026-08-21 共 17,553 条 Gold 成员日记录。乘数没有同日快照、结算价或持仓量缺失时保持不可用；席位市值仍因大商所排名覆盖失败而阻塞。 |
