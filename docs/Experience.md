...
- 通达信 `.day` 的价格缩放不能只由文件扩展名或市场决定。普通股票/指数、ETF/LOF/REIT、转债/回购可能使用不同倍率；任何新资产类型都要用同日公开数据或分钟线样本交叉验证后再固化倍率。
- 通达信日线的 32 位成交量字段可能在大成交量标的上从股/份切换为手。标准库必须定义统一单位，并用成交额、典型价格和成交量的数量级关系做质量检查，不能机械地全量乘 100。
- TickDB `volume` 的单位需要按市场和品种验证；`quote_volume` 可映射为成交额，但字段名相同不代表单位相同。原始值、单位推断和标准化值应可追溯。
- 同一标的的 TickDB、通达信和其他 Provider 记录要用来源后缀物理隔离。跨来源对照用于质量审计，不能把不同来源字段拼成一根无法追溯的 K 线。
...
1. 当前开发从根目录最新轮次计划启动；截至 2026-08-24 为 `Plan_R3.md`。同时阅读 ADR、`docs/CONTEXT.md` 和本文件中的相关环境事实。
